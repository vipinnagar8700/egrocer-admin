   ...
   public static function createOrderOnStripe($amount)
{
    ...
    public static function createOrderonPhonepe($type, $order_id, $wallet_amount)
    {
        try {
            ...
            // }
            $phonepay_client_id = Setting::get_value('phonepay_client_id');
            $phonepay_client_version = Setting::get_value('phonepay_client_version');
            $phonepay_client_secret = Setting::get_value('phonepay_client_secret');
            $explode =  $order_details['receipt'];
            $data = [
                // 'merchantId' => $phonepay_merchant_id,
                // 'merchantUserId' => $order_details['receipt'],
                // 'merchantTransactionId' => $merchantTransactionId,
                ...
                // 'redirectMode' => 'POST',
                // 'callbackUrl' => route('phonepe.callback'),
                // 'mobileNumber' => '913454353456',
                // 'paymentInstrument' => [
                //     'type' => 'PAY_PAGE',
                // ],
                'clientId' => $phonepay_client_id,
                'clientVersion' => $phonepay_client_version,
                'clientSecret' => $phonepay_client_secret,
                'amount' =>$order_amount * 100,
                'userId' => auth()->user()->id,
                'orderId' => $order_id,
            ];
            ...
            // Generate checksum (SHA256 + Salt) and append ###1
            $saltKey =  $phonepay_api_key; // Use your actual salt key from PhonePe
            $checksum = hash('sha256', $base64Payload . '/pg/v1/pay' . $saltKey) . '###1';
            $finalResponse = [
                'payload' => $data,
                // 'checksum' => $checksum,
                // 'appId' => 'Appid',
                'environment' => $phonepay_mode,
            ];

            return CommonHelper::responseWithData($finalResponse);
        } catch (\Exception $e) {
